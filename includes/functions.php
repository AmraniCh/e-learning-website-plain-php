...
***********************************/
    ...
    /*function get_more_groups(){
        global $con;

        $res = mysqli_query($con, "SELECT * FROM groupe WHERE pseudo_prof = '$username' LIMIT 3 ");
        while($row = mysqli_fetch_assoc($res)){
            return echo '<tr><td id="img-group-td"><img id="group-pic" src="assets/groups-images/'.$row['image_groupe'].'" class="img-responsive" style="height:60px;border-radius:50%"><style scoped>@media (max-width:768px){
                                                        #group-pic{height:auto!important;}
                                                        }</style></td><td id="nom-group-td">'.$row['nom'].'</td><td>'.$row['description'].'</td>
                                                        <td>'.$row['date_creation'].'</td>
                                                        <td>'.$row['nbr_fichier'].'</td>
                                                        <td>'.$row['nbr_etudient'].'</td>
                                                        <td class="code-td" onclick="copyvalue(this)"><code class=".code" id="codeid">'.$row['groupe_cle'].'</code>
                                                        ...
                                                        <td><button style="color: #fff;background-color: #138496; border: none;" type="button" id="btn-edit" class="btn btn-primary">Edit</button></td>
                                                        <td><button style="color: #fff;background-color: #dc3545; border: none;" type="button" id='.$row['id'].' class="btn-delete btn btn-danger">delete</button></td></tr>';
        }
        return null;
